...
public class Ghost : MonoBehaviour
{
    ...
    //public float slowModeMoveSpeed = 2.9f;
    ...
    private float slowerModeTimer = 0;
    ...
    void ModeUpdate()
    {
        if (currentMode == Mode.Slowed)
        {
            slowerModeTimer += Time.deltaTime;

            if (slowerModeTimer >= frightenedModeDuration)
            {
    ...
    void ChangeMode (Mode m)
    {
        ...
        {
            previousMoveSpeed = moveSpeed;
            moveSpeed = frightenedModeMoveSpeed/2;
        }
        if(currentMode != m)
        {
    ...
    Vector2 GetPinkGhostTargetTile()
    {
        ...
        //-Taking account position and Orietation
        Vector2 pacManPosition = pacMan.transform.localPosition;
    ...
    Vector2 GetOrangeGhostTargetTile()
    {
        ...
        //If the distance is greater than 8 tiles targeting is the same as Blinky
        //If the distance is less than 8 tiles, than target is his home node, so same as scatter mode.
        Vector2 pacManPosition = pacMan.transform.localPosition;
